...
public class QuestionRepository(AppDbContext db) : IQuestionRepository
{
    ...
    public async Task<bool> UpdateAsync(int pollId, int questionId, Question updatedQuestion, CancellationToken token = default)
    {

        var foundQuestion = await db.Questions
            .Include(q => q.Answers)
            .FirstOrDefaultAsync(q => q.PollId == pollId && q.Id == questionId, token);
        ...
        foundQuestion.Content = updatedQuestion.Content;
    ...
    public async Task<bool> ExistByContentWithDifferentId(int pollId, int questionId, string content, CancellationToken token = default)
        => await db.Questions.AnyAsync(q => q.PollId == pollId && q.Id != questionId && q.Content == content);
