...
public class UserSubmissionsRepository(AppDbContext db, HybridCache cache) : IUserSubmissionsRepository
{
    private const string SurveyStatisticsCacheKeyPrefix = "SurveyStatistics";


    public async Task<bool> IsSubmittedBeforeAsync(int surveyId, string userId, CancellationToken token = default)
    ...
    public async Task<bool> AddAsync(UserSubmission submission, CancellationToken token = default)
    {
        ...
        await cache.RemoveAsync($"{SurveyStatisticsCacheKeyPrefix}_{submission.SurveyId}", token);
    ...
    {
        return await db.UserSubmissions
            .AsNoTracking()
    ...
    public async Task<List<SurveyStatistics>> GetSurveyStatistics(int surveyId, CancellationToken cancellationToken = default)
    {
        ...
        return await cache.GetOrCreateAsync(
            $"{SurveyStatisticsCacheKeyPrefix}_{surveyId}",
            async cancel =>
            {
                Dictionary<int, int> countDictionary = db.SubmissionDetails
                    .Include(s => s.Question)
                    .Where(d => d.Question.SurveyId == surveyId)
                    .GroupBy(s => s.OptionId)
                    .Select(g => new { optionId = g.Key, count = g.Count() })
                    .ToDictionary(u => u.optionId, u => u.count);
