...
public interface ISurveyService : IScopedService
{
    ...
    Task<Result<ICollection<SurveyResponse>>> GetAllIncludingDeletedAsync(CancellationToken token = default);
    ...
    Task<Result> RestoreSurveyAsync(int surveyId, CancellationToken token = default);
