...
CancellationToken token = default)
    ...
    [HttpPut("{questionId}")]
    public async Task<IActionResult> UpdateQuestion([FromRoute] int pollId,
        [FromRoute] int questionId, UpdateQuestionRequest updateRequest,
        CancellationToken token = default)
    {
        Result result = await questionService.UpdateQuestionAsync(pollId, questionId, updateRequest, token);
        if (result is SuccessResult) return NoContent();
        return result.ToProblem();
    }
}
